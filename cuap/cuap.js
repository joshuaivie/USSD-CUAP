...
Session.prototype.send = function (pdu, responseCallback, sendCallback) {
	...
	let writePDU = pdu.toBuffer()

	this.socket.write(writePDU, function () {
		this.emit('send', pdu);
		if (sendCallback) {
			sendCallback(pdu);
		}
...
let createShortcut = function (command) {
	...
	return function (options, responseCallback, sendCallback) {
		...
		let pdu = new PDU(command, options);
		return this.send(pdu, responseCallback, sendCallback);
	};
};

for (let command in defs.commands) {
...
function Server(options, listener) {
	...
	transport.Server.call(this, options, function (socket) {
		let session = new Session({ socket: socket });
		session.server = self;
