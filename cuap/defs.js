...
gsmCoder.encode = function (string, encoding) {
  let coder = this.getCoder(encoding);
  let extCharsEnc = coder.extCharsEnc || coder.extChars;
...
filters.time = {
  ...
  decode: function (value) {
    ...
    let century = ('000' + new Date().getUTCFullYear()).substr(-4, 2);
    let result = new Date(value.replace(
      /^(..)(..)(..)(..)(..)(..)(.)?.*$/,
...
filters.message = {
  ...
  decode: function (value, skipUdh) {
    ...
    let udhi = this.esm_class & (consts.ESM_CLASS.UDH_INDICATOR || consts.ESM_CLASS.KANNEL_UDH_INDICATOR);
    let result = {};
...
let commands = {
  bind: {
    id: 0x00000065,
    params: {
      system_id: { type: types.cstring, size: 11 },
      password: { type: types.cstring, size: 9 },
      system_type: { type: types.cstring, size: 13 },
      interface_version: { type: types.int32, default: 0x00000010, size: 4 },
    }
  },
  bind_resp: {
    id: 0x00000067,
